...
class MQTTClient:
    ...
    def subscribe(self, topic, qos=0):
        ...
        pid = next(self.newpid)
        self.rcv_pids[pid]=pid #HGODE add pid to dict
        print('subscribe: pid='+str(pid))
        sz = 2 + 2 + len(topic) + 1
        plen = self._varlen_encode(sz, pkt, 1)
    ...
    def check_msg(self):
        ...
        if op == 0x90:  # SUBACK Packet fixed header
            ...
            if resp[3] not in (0, 1, 2):
                raise MQTTException(40, resp)
            pid = resp[2] | (resp[1] << 8)
            assert pid is not None, "check_msg(): pid is not set"
            print('Looking for pid: '+str(pid))
            assert pid in self.rcv_pids, "check_msg(): pid is not in rcv_pids"
